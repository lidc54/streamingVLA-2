...
import lerobot.datasets.normalize as normalize
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.delta_timestamps is not None:
            ...
            self.delta_indices['observation.state'] = self.delta_indices['action'] + [len(self.delta_indices['action'])]
            # import pdb; pdb.set_trace()
            # self.delta_indices['action'] += [len(self.delta_indices['action'])]
        self.get_all_action()
    ...
    def get_all_action(self):
        ...
        for ep_idx in range(len(self.episode_data_index["from"])):
            ep_start = self.episode_data_index["from"][ep_idx]
            ep_end = self.episode_data_index["to"][ep_idx]
            q_idx = torch.arange(ep_start, ep_end).long()
            ep_idx_act = torch.stack(self.hf_dataset.select(q_idx)['action'])
            ep_act = torch.cat([torch.zeros((1,7),device=ep_idx_act.device), ep_idx_act], dim=0)
            ep_act = torch.cumsum(ep_act, dim=0)
            self.all_action[ep_idx] = ep_act[:-1]
            stats.update(ep_idx_act.cpu().numpy())
        # import pdb; pdb.set_trace()
        all_action_stats = stats.get_statistics()
        q01, q99 = all_action_stats.q01, all_action_stats.q99
        scale = np.maximum(np.abs(q99), np.abs(q01)) + 1e-6
        self.meta.stats['action']['mean'] *= 0
        self.meta.stats['action']['std'] = scale
        self.meta.stats['observation.state']['mean'] *= 0
        # self.meta.stats['observation.state']['std'] = 0
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if self.delta_indices is not None:
            ...
            for key, val in query_result.items():
                item[key] = val
            act_query_index = torch.LongTensor(query_indices['observation.images.image']) - self.episode_data_index["from"][ep_idx]
            item['action_states'] = self.all_action[ep_idx][act_query_index] / self.meta.stats['action']['std']

        # if padding['action_is_pad'][0]: item['action'][0] *= 0
        if len(self.meta.video_keys) > 0:
            video_error, msg=0,''
            ...
            while video_error < 3:
                try:
                    query_timestamps = self._get_query_timestamps(current_ts, query_idx)
                    video_frames = self._query_videos(query_timestamps, ep_idx)
                    break
                except Exception as e:
                    video_error+=1
                    msg+=f'error:{video_error}: {e}\n\n'
                    #从前面取数据帧
                    query_idx, _ = self._get_query_indices(idx-1-video_error, ep_idx)
            else:
                raise Exception(f'{"#"*9}\n{msg}video_error: {video_error}, idx: {idx}, ep_idx: {ep_idx}\n\n{"#"*9}')
